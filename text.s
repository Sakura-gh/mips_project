...
# 输入格式：寄存器编号 DREG，如: 12 DREG -> 显示第12号寄存器的值
display_regs:
addi $sp, $sp, -12
sw $ra, 8($sp)
sw $a0, 4($sp)
...
sw $zero, 0($t0)            # 用一个空格把参数、指令和函数的输出分隔开来
addi $t0, $t0, 4
lw $s1, 0($t1)              # 取出低位值$s1
...
bne $s1, $s2, B2D_return    # 如果第一个字符不是'B'，则直接跳出检测
addi $t1, $t1, 4            # 地址$t1++，检测下一个字符
B2D_1:
lw $s1, 0($t1)              # 取出$t1地址上的字符
ori $s2, $zero, 0x0F32      # $s2 = '2'
bne $s1, $s2, B2D_return    # 如果第二个字符不是2'，则直接跳出检测
addi $t1, $t1, 4            # 地址$t1++，检测下一个字符
B2D_2:
lw $s1, 0($t1)              # 取出$t1地址上的字符
ori $s2, $zero, 0x0F44      # $s2 = 'D'
bne $s1, $s2, B2D_return    # 如果第三个字符不是'D'，则直接跳出检测
addi $t1, $t1, 4            # 地址$t1++，检测下一个字符
B2D:
bne $t1, $t0, B2D_return    # 如果前三个字符不是'B2D'，则直接返回，而不显示任何内容
add $t1, $zero, $s6         # 以下循环是为了获取当前行行首地址
B2D_addr:
addi $t1, $t1, 320
slt $t3, $t1, $t0
bne $t3, $zero, B2D_addr
